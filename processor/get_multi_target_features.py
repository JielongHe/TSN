...
from processor.rerank_for_cluster import re_ranking, re_ranking_gpu
...
def euclidean_distance_matrix(feature):
    # 计算欧式距离
    pairwise_distances = torch.cdist(feature, feature, p=2)
    return pairwise_distances


def get_distances(source_feather, target_feather, order=-1, ratio=0.003, domain=0, flag='all'):
    logger = logging.getLogger("IRRA.train")
    logger.info('get_distances:')
    source_feather = source_feather.to('cpu').numpy()
    target_feather = target_feather.to('cpu').numpy()

    rerank_dist = re_ranking(source_feather, target_feather, lambda_value=0.1)
    # rerank_dist = re_ranking_gpu(source_feather, target_feather, lambda_value=0.1)
    # rerank_dist = intra_distance(all_feather)
    # rerank_dist = euclidean_distance_matrix(all_feather)
    # DBSCAN cluster
    ...
    logger.info('%s eps in cluster: %.3f' % (flag, eps))
    eps_list = [0]
    for i in [1, 2, 5, 10, 20, 50, 100, 200, 500, 1000]:
        print('i = %5d   %.3f' % (i, tri_mat[:int(0.001 * i * tri_mat.size)].mean()))
        logger.info('i = %5d   %.3f' % (i, tri_mat[:int(0.001 * i * tri_mat.size)].mean()))
