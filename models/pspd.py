...
class Img_Cross_Model(nn.Module):
    def __init__(self, embed_dim):
        self.embed_dim = embed_dim
        super(Img_Cross_Model, self).__init__()
...
class PSPD(nn.Module):
    def __init__(self,
        ...
        self.cross_proj = nn.Linear(vision_width, self.embed_dim)

        self.vision_proj = nn.Linear(vision_width, self.embed_dim)
        self.text_proj = nn.Linear(text_width, self.embed_dim)

        self.image_cross_model = Img_Cross_Model(vision_width)
    ...
    def forward(self, image, caption, gpt_captions, idx):
        ...
        with torch.no_grad():
            text_output1 = self.text_encoder(text.input_ids, attention_mask=text.attention_mask,
                                            return_dict=True, mode='text')
            text_embeds1 = text_output1.last_hidden_state

            image_embeds1 = self.visual_encoder(image)

        cross_embeds = self.image_cross_model(image_embeds1, text_embeds1, text_embeds1)

        cross_feats = self.cross_proj(cross_embeds[:,0,:])

        cross_logits = self.classifier(cross_feats).float()
        text_logits = self.classifier(text_feats).float()

        id_loss = objectives.compute_id(cross_logits, text_logits, idx)
        # ret.update(id(image_log
        #         #     {'id_loss': objectives.compute_its, text_logits, batch['pids']) * self.args.id_loss_weight})
        ...
        return global_loss, local_loss, id_loss
    ...
    def itm_loss1(self, image_embeds, text, image_feat, text_feat, idx):
        ...
        # fuse_feats = output_pos.last_hidden_state[:, 0, :]
        #
        # fuse_logits = self.itm_id_head(fuse_feats).float()
        # # text_logits = self.classifier(text_feats).float()
        # image_logits = None
        #
        # id_itm_loss = objectives.compute_id(image_logits, fuse_logits, itm_id)
